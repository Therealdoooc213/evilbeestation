...
export class MalfunctionModulePicker extends Component {
  ...
  render() {
    ...
    const { ref } = config;
    const { compact_mode, processing_time, categories = [] } = data;
    const { hoveredItem, currentSearch } = this.state;
    return (
      <Section
        title={
          <Box inline color={processing_time > 0 ? 'good' : 'bad'}>
        ...
        buttons={
          <Fragment>
            Search
            <Input value={currentSearch} onInput={(e, value) => this.setSearchText(value)} ml={1} mr={1} />
            <Button
            ...
            <ItemList
              ...
              items={categories
                ...
                .filter((item) => {
                  ...
                  const searchableString = String(item.name + item.desc).toLowerCase();
...
const ItemList = (props) => {
  const { items, hoveredItem, processing_time, compact, onBuy, onBuyMouseOver, onBuyMouseOut } = props;
